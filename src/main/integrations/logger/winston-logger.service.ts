...
export class WinstonLoggerService extends CommonLogger {
  ...
  private getValueContext<K extends keyof ContextKeys>(context: K, key: keyof ContextKeys[K]) {
  ...
  private consoleFormat() {
    return printf(({ level, message, timestamp }) => {
      ...
      const addMessage = (color: keyof typeof colors, value: string | null, bracket = true) => {
        if (!value) return
